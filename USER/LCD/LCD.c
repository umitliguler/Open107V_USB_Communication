...
{
  LCD_DrawLine( Xpos , Ypos, Xpos + Width, Ypos ,TextColor );
  LCD_DrawLine( Xpos , Ypos + Height, Xpos + Width, Ypos + Height ,TextColor );
  LCD_DrawLine( Xpos , Ypos, Xpos , Ypos + Height ,TextColor );
  LCD_DrawLine( Xpos + Width, Ypos, Xpos + Width, Ypos + Height ,TextColor );
}

/******************************************************************************
* Function Name  : LCD_DrawFullRect
* Description    : Displays a full rectangle.
* Input          : - Xpos: specifies the X position.
*                  - Ypos: specifies the Y position.
...
*                          This parameter can be one of the following values 0..9
...
{
  uint16_t refcolumn = 320 - 1;

  /* Send the string character by character on lCD */
  while ((*ptr != 0) & (((refcolumn + 1) & 0xFFFF) >= 16))
  {
    if( *ptr < 0x80)	  /* ASCI */
	{
	  /* Display one character on LCD */
  	  PutChar( 319 - refcolumn , Line * 16 , *ptr , TextColor , BackColor);	    
	...
	{ 
	  PutChinese( 319 - refcolumn ,Line * 16 , ptr++ , TextColor , BackColor);	
	  /* Decrement the column position by 8 */
      refcolumn -= 16;
      /* Point on the next character */
	  ptr++;
